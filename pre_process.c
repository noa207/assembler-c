...
int pre_process(char *filename) {
  ...
  char mcro_names[MAX_LINE_LENGTH] = {0};
  char *mcro_line = NULL;
  ...
  while (fgets(line_of_file, sizeof(line_of_file), file) != NULL){ /*read each line in the file*/
    ...
    if (strncmp(line_of_file, "mcro", 4) == 0 && strncmp(line_of_file, "mcroend", 7) != 0){ /*how its work? i didnt understand*/
      if (checking_mcro_name(tokens[1]) && mcro_line_valid(tokens, 2)) { /*check if is macro name of the define valid and there is no other words after it*/
          flag = 1;
          strcpy(mcro_names,line_of_file + 4);
      ...
      fprintf(stderr,"STRING ERROR: Unwanted chars before mcro\n");     
        ...
        flag = 0;
        if (mcro_line != NULL) {
            saving_mcro_data(mcro_names, mcro_line); /*how its work? i didnt understand*/
        }
        else {
...
void create_am_file(char *input_filename, char *output_filename){
  ...
  while(fgets(line_of_file, sizeof(line_of_file), file_input) != NULL){
    ...
    Subtracting_spaces(result.part1); /*need to change*/
    char *mcro_lines = replacing_mcro_name(result.part1); /*need to change*/
    if (mcro_lines != NULL) {
...
boolean checking_mcro_name(char *mcro_name) {
  ...
  for (i = 0 ; i < number_of_mcros; i++) { /*check if it is elready appends*/
    if (strcmp(mcro_name, mcros[i].mcro_name) == 0) {
